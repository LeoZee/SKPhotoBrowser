...
open class SKPhotoBrowser: UIViewController {
	...
	open func getToolbar() -> UIToolbar {
		return toolbar
	}
	
	open func getPaginationViewFrame() -> CGRect {
		return paginationView.frame
	}
	
	@objc open func actionButtonPressed(ignoreAndShare: Bool) {
		...
		if let titles = SKPhotoBrowserOptions.actionButtonTitles {
			...
			for idx in titles.indices {
				actionSheetController.addAction(UIAlertAction(title: titles[idx], style: .default, handler: { (_) -> Void in
					self.delegate?.didDismissActionSheetWithButtonIndex?(idx, photoIndex: self.currentPageIndex, browser: self)
				}))
			}
			...
			self.delegate?.didDismissActionSheetWithButtonIndex?(0, photoIndex: self.currentPageIndex, browser: self)
		}
	}
